...
function text(data) {
    currentObject[textNodeName()] =
        (currentObject[textNodeName()] || '') + data;
}
...
function endElement(name) {
    if (currentObject[textNodeName()]) {
        if (options.trim) {
            currentObject[textNodeName()] =
                currentObject[textNodeName()].trim();
        }
        ...
        currentObject[textNodeName()] = coerce(
            currentObject[textNodeName()],
            name
        );
    }
    ...
    if (!options.reversible) {
        if (
            textNodeName() in currentObject &&
            Object.keys(currentObject).length == 1
        ) {
...
function textNodeName() {
    return options.alternateTextNode
        ? typeof options.alternateTextNode === 'string'
            ? options.alternateTextNode
            : '_t'
        : '$t';
}
...
module.exports = function (xml, _options) {
    ...
    var schema = joi.object({
        ...
        arrayNotation: joi
            .alternatives([joi.boolean(), joi.array()])
            .default(false),
        alternateTextNode: [
    ...
    if (!parser.parse(xml)) {
        throw new Error(
            'There are errors in your xml file: ' + parser.getError()
        );
    }
